...
from src.market_expert import SOURCE_NOTE, MarketResearchReport, run_market_research_expert


st.set_page_config(
    page_title="Meta Product Opportunity Research Agent",
    page_icon="M",
    layout="wide",
)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        run_mode_label = st.radio("运行模式", list(RUN_MODE_OPTIONS), index=0)
        ...
        if run_mode == "local_demo":
            st.success("本地 Demo 模式：不调用任何 API。")
        else:
            st.info("联网搜索模式：可直接在 Demo 中输入 API 配置；这些值只在本次运行中使用，不会写入代码。")
            ...
            with st.expander("三模型角色配置", expanded=True):
            ...
            st.caption("每个模型会以不同角色联网搜索分析同一个产品想法，并在每个页面用卡片展示各自结果。AIHubMix :surfing 模型会走 Chat Completions 搜索增强。")
            if has_api_key(api_key):
                st.success("已检测到 API Key。输入框可以留空，系统会自动读取本地配置。")
            else:
        ...
        local_file_path = st.text_input(
            "本地文件路径（上传失败时使用）",
            value="",
            placeholder=r"例如：E:\vibecoding\scepter-product-definition.md",
            help="如果浏览器上传一直显示 Connection lost，可以把同一文件的本地完整路径粘贴到这里。",
        )
        ...
        st.caption(
            "你不需要提供完整 PRD。只要输入一个初步产品想法，系统会从痛点、用户、市场、竞品、地区、商业化、落地风险和 MVP 验证八个维度，帮助你判断这个想法是否具有市场机会。"
        )
        start = st.button("开始产品机会研究", type="primary", use_container_width=True)

    _render_role_intro((api_model_1, api_model_2, api_model_3), run_mode)
    ...
    if not start:
        with results_slot.container():
            _render_saved_results(st.session_state["research_result_store"])
        return

    try:
        idea_text, source = choose_prd_text(uploaded_file, pasted_text, local_file_path)
    ...
    with st.spinner("正在生成结构化机会评分与 MVP 验证补充..."):
        report = run_market_research_expert(product_name=product_name, product_type=product_type, idea_text=idea_text)
...
def _render_saved_results(result_store: dict, preferred_key: str | None = None) -> None:
    ...
    keys = [key for key in [preferred_key, "standard", "explore", "local_demo"] if key and key in result_store]
    keys.extend(key for key in result_store if key not in keys)

    if len(keys) == 1:
        _render_result_payload(result_store[keys[0]])
        return

    tabs = st.tabs([_result_store_label(key) for key in keys])
    for tab, key in zip(tabs, keys):
        with tab:
            _render_result_payload(result_store[key])
...
def _render_result_payload(payload: dict) -> None:
    ...
    st.success(f"研究完成。输入来源：{source} · 运行模式：{run_mode_label} · 研究结构：{research_structure_label}")
    if failed_outputs:
    ...
    if web_result is not None and research_structure == "explore":
        ...
        with tabs[-1]:
            _render_final_report(report=None, web_result=web_result)
    else:
...
这是一个“早期产品机会研究 Agent / Early-stage Product Opportunity Research Agent”。它不是 PRD 完整性检查器，而是用于判断一个产品想法是否值得继续推进的市场研究工具。
        """
    )


def _render_role_intro(model_names: tuple[str, str, str], run_mode: str) -> None:
    if run_mode == "local_demo":
        model_names = ("本地 Demo 规则", "本地 Demo 规则", "本地 Demo 规则")
    roles = [
...
<div class="model-detail-header {role_class}">
  ...
  <div class="model-detail-model">{escape(output.model or "未配置模型")}</div>
    ...
    st.markdown('<div class="explore-result-block">', unsafe_allow_html=True)
    if output.error:
        st.error(output.error)
    elif output.content:
        st.markdown(_format_citation_links(output.content))
    else:
  ...
  <div class="model-detail-model">{escape(output.model or "未配置模型")}</div>
    ...
    with st.expander("展开 / 收起详细调研结果", expanded=True):
        if output.error:
            ...
            return
        if section:
            st.markdown(_format_citation_links(section))
        elif output.content:
...
def _render_final_report(report: MarketResearchReport | None, web_result=None) -> None:
    ...
    st.text_area("可复制报告正文", report_text, height=760)
...
<style>
  ...
  .explore-result-block h1 { font-size: 1.16rem !important; }
  .explore-result-block h2 { font-size: 1.08rem !important; }
  .explore-result-block h3,
  .explore-result-block h4 { font-size: 0.96rem !important; }
